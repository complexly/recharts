...
setPolar <- function(chart, center=c('50%', '50%'), radius='75%', startAngle=90,
                     splitNumber=5, boundaryGap=c(0, 0), scale=FALSE, axisLine=NULL,
                     axisLabel=NULL, splitLine=NULL, splitArea=NULL,
                     type=c('polygon', 'circle'), indicator=NULL, axisName=NULL,
                     ...){
    ## FIXME: Will develop later.
    stopifnot(inherits(chart, 'echarts'))
...
setAes <- function(
    ...
    if (! is.null(bgColor) && bgColor != 'rgba(0,0,0,0)'){
    ...
    if (!animation) lst[['animation']] <- FALSE
    ...
    if (!is.null(animationDuration) && animationDuration != 2000){
        ...
        if (hasZ)
            chart$x$timeline[['playInterval']] <- animationDuration
    }
...
#' Set Theme of Echarts
#'
#' Set the theme of Echarts. You can use a pre-installed theme or define you own themes. \cr
#' When an echart object is generated, you can modify it by setting theme using
#' \code{\link{\%>\%}}.
#'
...
#' 'green', 'helianthus', 'macarons2', 'mint', 'red', 'roma', 'sakura', 'shine', 'vintage'} \cr
...
#' @export
#'
#' @references \url{http://echarts.baidu.com/echarts2/doc/example/theme.html}
#' @examples
#' \dontrun{
#' g <- echartR(iris, Sepal.Width, Petal.Width, series=Species)
#'
...
#' g %>% setTheme(theme)
#' }
setTheme <- function(chart, theme=c(
    'default', 'macarons', 'infographic', 'blue', 'dark', 'gray', 'green',
